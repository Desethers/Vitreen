...
function fullArtworkHtml(img: ImageItem | null, block: Block, inquireHref: string, extraClass = ''): string {
  return `
...
export function generateBlocksPDF({
  ...
  const pages = [cover, ...blocks.map(b => blockHtml(b, images, inquireHref)).filter(Boolean)].join('\n')
