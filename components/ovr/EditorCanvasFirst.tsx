...
function normalizeBlock(raw: unknown): Block | null {
  ...
  if (
    ...
    !BLOCK_TYPES.has(o.type as BlockType)
  )
    return null;
  ...
  return (
    <div
      ...
      className={`fixed inset-0 flex flex-col items-center justify-center transition-colors duration-300 ${
        over ? "bg-gray-100 dark:bg-[#181818]" : "bg-gray-50 dark:bg-[#111111]"
      }`}
    >
      <input
        ref={ref}
      ...
      <div className="relative z-10 max-w-xl px-6 text-center">
        ...
        <p className="hero-fade-up hero-fade-up-delay text-[15px] text-gray-500 dark:text-gray-400 leading-relaxed mb-10 max-w-md mx-auto">
          From <strong className="font-medium text-gray-700 dark:text-gray-300">5 images</strong> to
          a viewing room your collector wants to open — in two seconds.
        </p>
      ...
      <div className="grid h-[37px] w-full grid-cols-3 items-center gap-1 rounded-[5px] border border-gray-200/70 bg-white/95 px-1 shadow-[0_8px_30px_rgba(0,0,0,0.08)] backdrop-blur-xl dark:border-gray-800 dark:bg-[#1c1c1c]/95 md:flex md:h-[37px] md:w-auto md:rounded-[5px] md:pl-2 md:pr-1">
        ...
        >
          ...
          <span className="relative h-3 w-3 shrink-0 overflow-hidden" aria-hidden>
            <svg
              className="absolute inset-0 transition-all duration-200 group-hover:translate-x-1 group-hover:opacity-0"
              width="12"
              height="12"
              fill="none"
              stroke="currentColor"
            ...
            <svg
              className="absolute inset-0 -translate-x-1 opacity-0 transition-all duration-200 group-hover:translate-x-0 group-hover:opacity-100"
              width="12"
              height="12"
              fill="none"
              stroke="currentColor"
...
function PanelAddTextMenu({
  atIndex,
  onPick,
}: {
  atIndex: number;
  onPick: (index: number, kind: "quote" | "text") => void;
}) {
  return (
...
function TextsSection({
  ...
  const updateQuote = (id: string, patch: Partial<QuoteItem>) => {
    onChange({
      ...textPool,
      quotes: textPool.quotes.map((q) => (q.id === id ? { ...q, ...patch } : q)),
    });
  };
  ...
  const onQuotesDragEnd = (result: DropResult) => {
    ...
    if (
      result.source.droppableId !== "text-pool-quotes" ||
      result.destination.droppableId !== "text-pool-quotes"
    )
      return;
  ...
  return (
    <section>
      <div className="mb-3 flex items-baseline justify-between">
        <h3 className="text-[11px] uppercase tracking-[0.15em] text-gray-400 dark:text-gray-600">
          Texts
        </h3>
      ...
      <div className="mb-3">
        {textPool.quotes.length === 0 ? (
          <div className="rounded-lg border border-dashed border-gray-200 py-7 text-center dark:border-gray-700">
            <p className="mb-3 px-4 text-[12px] text-gray-500 dark:text-gray-400">
              Quotes & texts — drag to reorder.
            </p>
            <div className="inline-flex gap-2 px-4">
              ...
              >
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-gray-100 text-[11px] dark:bg-gray-700">
                  +
                </span>
                Quote
              </button>
                          ...
                          >
                            ...
                            <div
                              className={`group/q space-y-1.5 rounded-lg border border-gray-200 bg-gray-50/50 p-2.5 transition-colors dark:border-gray-800 dark:bg-gray-900/30 ${
                                snapshot.isDragging
                                  ? "border-gray-400 shadow-lg ring-1 ring-gray-900/10 dark:border-gray-600 dark:ring-white/10"
      ...
      <div>
        {closingOpen || textPool.closing ? (
          <div className="space-y-1.5">
            <p className="px-1 text-[10px] uppercase tracking-wider text-gray-400 dark:text-gray-600">
              Closing word
            </p>
            <textarea
              className={inputCls}
          ...
          >
            <span className="flex h-4 w-4 items-center justify-center rounded-full bg-gray-100 text-[11px] dark:bg-gray-800">
              +
            </span>
            Add a closing word
          </button>
...
function SettingsDrawer({
  ...
  const inputCls =
    "w-full border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-[13px] bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-gray-900/8 focus:border-gray-400 transition-colors";
    ...
    <>
      ...
      >
        <div className="shrink-0 flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-800">
          <span className="text-[15px] font-medium text-gray-900 dark:text-gray-100 tracking-tight">
            Settings
          </span>
          <button
            type="button"
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
          <section>
            <h3 className="text-[11px] uppercase tracking-[0.15em] text-gray-400 dark:text-gray-600 mb-3">
              Recipient
            </h3>
          ...
          <section>
            ...
            <div className="space-y-2.5">
              ...
              <textarea
                className={inputCls}
                placeholder="Personal greeting (shown on cover)"
                rows={3}
          ...
          <section>
            ...
            <div className="grid grid-cols-3 gap-2">
              {images.map((img) => (
                <div
                  key={img.id}
                  className="relative group/img aspect-square rounded-lg overflow-hidden border border-gray-200 dark:border-gray-800"
                >
                  {img.dataUrl ? (
          ...
          <section>
            ...
            <div className="space-y-2.5">
              <input
                ...
                onChange={(e) => set("galleryAddress", e.target.value)}
              />
              <input
                className={inputCls}
                placeholder="Contact"
...
export default function EditorCanvasFirst() {
  ...
  useLayoutEffect(() => {
    try {
      ...
      if (i) {
        ...
        if (Array.isArray(p))
          setImages(p.map(normalizeImage).filter((x): x is ImageItem => x !== null));
      }
      const b = sessionStorage.getItem("vr_blocks");
      if (b) {
        const p = JSON.parse(b) as unknown;
        if (Array.isArray(p))
          setBlocks(p.map(normalizeBlock).filter((x): x is Block => x !== null));
      }
  ...
  const saveTextPool = useCallback((tp: TextPool) => {
    setTextPool(tp);
    try {
      sessionStorage.setItem("vr_text_pool", JSON.stringify(tp));
    } catch {
      /* ignore */
    }
  }, []);
  ...
  const insertTextBlock = useCallback((index: number, kind: "quote" | "text") => {
    ...
    setBlocks((prev) => {
      ...
      try {
        sessionStorage.setItem("vr_blocks", JSON.stringify(next));
      } catch {
        /* ignore */
      }
      return next;
  ...
  const insertImageBlock = useCallback((index: number, imageId: string) => {
    setBlocks((prev) => {
      ...
      const restIds = (src.slots.map((s) => s.imageId).filter(Boolean) as string[]).filter(
        (id) => id !== imageId
      );
      const withoutImage = prev.flatMap((block, blockIndex) => {
        ...
        const type: BlockType =
          restIds.length === 1 ? "full" : restIds.length === 2 ? "pair" : "trio";
  ...
  const updateImage = useCallback((id: string, patch: Partial<ImageItem>) => {
    setImages((prev) => {
      ...
      try {
        sessionStorage.setItem("vr_images", JSON.stringify(next));
      } catch {
        /* ignore */
      }
      return next;
  ...
  const mergeImageIntoQuote = useCallback((srcImageId: string, quoteBlockId: string) => {
    setBlocks((prev) => {
      ...
      const newQuote: Block = {
        ...q,
        type: "side",
        slots: [{ imageId: srcImageId }],
        sideTextType: "quote",
      };
      ...
      const next = prev.flatMap((b) => {
        ...
        if (b.id === src.id) {
          ...
          const srcType: BlockType =
            srcRest.length === 1 ? "full" : srcRest.length === 2 ? "pair" : "trio";
      ...
      const next = prev.flatMap((b) => {
        ...
        if (b.id === f.id)
          return [
            {
              ...f,
              type: "side" as BlockType,
              quoteText: q.quoteText,
              quoteAuthor: q.quoteAuthor,
              textStyle: q.textStyle,
              sideTextType: "quote" as const,
            },
          ];
        return [b];
      });
  ...
  const mergeImage = useCallback((srcImageId: string, dstBlockId: string) => {
    setBlocks((prev) => {
      ...
      const dstType: BlockType =
        newDstIds.length === 1 ? "full" : newDstIds.length === 2 ? "pair" : "trio";
      const newDst: Block = {
        ...dst,
        type: dstType,
  ...
  const mergeMoveBlock = useCallback((srcBlockId: string, dstBlockId: string) => {
    setBlocks((prev) => {
      ...
      const newType: BlockType =
        newIds.length === 1 ? "full" : newIds.length === 2 ? "pair" : "trio";
