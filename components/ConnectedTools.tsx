    ...
    >
      <div className="flex">
        ...
        >
          ...
          {SIDEBAR_ITEMS.map((item) => (
            <button
              type="button"
              key={item.label}
              className={`flex w-full cursor-pointer items-center gap-2 rounded-[4px] px-2 py-[5px] text-left text-[9px] transition-[background-color,color,transform,box-shadow] duration-150 hover:translate-x-[2px] hover:text-[#111110] ${
                item.active
                  ? glass
                    ? "bg-white/45 text-[#111110] hover:bg-white/70"
                    : "bg-white text-[#111110] hover:bg-white"
                  : glass
                    ? "text-[#6B6A67] hover:bg-white/45"
                    : "text-[#6B6A67] hover:bg-white hover:shadow-[inset_0_0_0_0.5px_#E8E8E6]"
              }`}
        ...
        <div
          className={
            glass
              ? `flex-1 min-w-0 bg-white/28 ${compact ? "px-3 py-3" : "min-h-[600px] px-5 py-4"}`
              : `flex-1 min-w-0 ${compact ? "px-3 py-3" : "min-h-[600px] px-5 py-4"}`
          }
        >
          {compact && (
            <div className="mb-3 flex items-center justify-between border-b-[0.5px] border-[#E8E8E6] pb-2">
              ...
              <span className="relative flex h-6 w-6 items-center justify-center rounded-full text-[11px] text-[#6B6A67]">
                🔔
                <span className="absolute -right-0.5 -top-0.5 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-[#1FA854] text-[7px] font-medium text-white">
                  3
                </span>
              </span>
            </div>
            ...
            <>
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    className="flex h-[22px] w-[22px] items-center justify-center rounded-[5px] border-[0.5px] border-[#E8E8E6] text-[10px] text-[#6B6A67]"
                    aria-label="Importer un CSV"
                  >
                    ↑
                  </button>
                  <button className="rounded-[5px] bg-[#111110] px-2 py-[4px] text-[8px] text-white">
                    + Ajouter
                  </button>
                </div>
              </div>
              <p className="mt-0.5 text-[8px] text-[#6B6A67]">
                {ARTWORK_ROWS.length} au total · {ARTWORK_ROWS.length} disponibles · 0 vendue
              </p>
            </>
          ) : (
          ...
          {/* Search */}
          <div
            className={`flex items-center gap-2 rounded-[5px] border-[0.5px] border-[#E8E8E6] bg-[#FAFAF8] px-2 py-[3px] ${
              compact ? "mt-2" : "mt-3"
            }`}
          >
            <span className="text-[9px] text-[#ADADAA]">⌕</span>
            <span className="flex-1 truncate text-[8.5px] text-[#ADADAA]">
          ...
          >
            {FILTER_PILLS.map((p) => (
              <span
                key={p}
                className={`flex-shrink-0 rounded-full border-[0.5px] border-[#E8E8E6] text-[#6B6A67] ${
                  compact
                    ? "whitespace-nowrap px-1.5 py-[1.5px] text-[7px]"
                    : "px-2 py-[2px] text-[7.5px]"
                }`}
              >
                {p}
              </span>
            ))}
            <span className="ml-1 h-3 w-px flex-shrink-0 bg-[#E8E8E6]" />
            <span
              className={`flex-shrink-0 rounded-full bg-[#111110] text-white ${
                compact
                  ? "whitespace-nowrap px-1.5 py-[1.5px] text-[7px]"
                  : "px-2 py-[2px] text-[7.5px]"
              }`}
          ...
          {compact && (
            <div className="mt-2.5 flex items-center justify-between">
              <span className="text-[7.5px] text-[#6B6A67]">{ARTWORK_ROWS.length} résultats</span>
              <div className="flex items-center gap-1">
                <button
                  className="flex h-5 w-5 items-center justify-center rounded-[4px] border-[0.5px] border-[#E8E8E6] text-[9px] text-[#6B6A67]"
                  aria-label="Vue grille"
                >
                  ▦
                </button>
                <button
                  className="flex h-5 w-5 items-center justify-center rounded-[4px] border border-[#111110] text-[9px] text-[#111110]"
          ...
          {/* Table header */}
          <div
            className={`mt-3 grid items-center border-b-[0.5px] border-[#E8E8E6] pb-1.5 uppercase tracking-[0.06em] text-[#ADADAA] ${
              compact
                ? "grid-cols-[140px_42px_34px_52px_50px] gap-1 text-[6.5px]"
                : wideTitle
                  ? "grid-cols-[3fr_1fr_60px_90px_80px] gap-2 text-[7.5px]"
                  : "grid-cols-[1fr_120px_60px_90px_80px] gap-2 text-[7.5px]"
            }`}
          >
            <span>Titre</span>
            <span>Artiste</span>
          ...
          <div className="flex flex-col">
            {(maxRows
              ? ARTWORK_ROWS.slice(0, maxRows)
              : compact
                ? ARTWORK_ROWS.slice(0, ARTWORK_ROWS.length - 6)
                : ARTWORK_ROWS
            ).map((row) => (
              <div
                key={row.title}
                className={`grid items-center border-b-[0.5px] border-[#F1F1ED] text-[#111110] ${
                  compact
                    ? "grid-cols-[140px_42px_34px_52px_50px] gap-1 py-1.5 text-[7px]"
                    : wideTitle
                      ? "grid-cols-[3fr_1fr_60px_90px_80px] gap-2 py-2 text-[9px]"
                      : "grid-cols-[1fr_120px_60px_90px_80px] gap-2 py-2 text-[9px]"
                }`}
              >
                ...
                <span className="truncate text-[#6B6A67]">{SAMPLE_ARTIST.fullName}</span>
                <span className="text-[#6B6A67]">{row.year}</span>
                <span className="text-right tabular-nums">{row.price}</span>
                <span className="overflow-hidden">
                  <span
                    className={`inline-block max-w-full truncate rounded-full border-[0.5px] border-[#1FA854]/40 bg-[#1FA854]/[0.06] px-1.5 py-[1px] text-[#1FA854] ${
          ...
          {/* Bottom nav */}
          {compact && (
            <div className="-mx-3 -mb-3 mt-3 flex items-center justify-between border-t-[0.5px] border-[#E8E8E6] px-2 py-2">
              {BOTTOM_NAV_ITEMS.map((item) => (
                <div
                  key={item.label}
                  className={`flex flex-col items-center gap-0.5 text-[7px] ${
                    item.active ? "text-[#111110]" : "text-[#ADADAA]"
                  }`}
                >
