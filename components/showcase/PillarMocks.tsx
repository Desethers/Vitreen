...
function GalleryOsSidebar({ active }: { active: NavId }) {
  return (
    <aside
      className="hidden md:flex shrink-0 flex-col bg-white border-r border-zinc-200"
      style={{ width: "20%" }}
    >
      {/* Logo */}
      <div className="px-3 py-3 border-b border-zinc-100">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] font-medium text-zinc-900 tracking-tight">Gallery OS</span>
        </div>
      </div>
      {/* Nav */}
      <nav className="flex-1 px-1.5 py-2 flex flex-col gap-[1px] overflow-hidden">
        {NAV.map((it) => {
          ...
          return (
            <div
              key={it.id}
              className={`flex items-center gap-1.5 px-1.5 py-[5px] rounded-[5px] text-[9px] ${isActive ? "bg-zinc-100 text-zinc-900 font-medium" : "text-zinc-700"}`}
            >
              <I size={10} className={isActive ? "text-zinc-900" : "text-zinc-600"} />
      ...
      {/* Footer */}
      <div className="px-3 py-2.5 border-t border-zinc-100 flex items-center justify-between">
        <p className="text-[7.5px] text-zinc-400">Powered by Vitreen</p>
        <p className="text-[7.5px] text-zinc-400">Déconnexion</p>
      </div>
    </aside>
  );
}
...
export function ArchiveMock() {
  return (
    <div className="w-full h-full bg-white font-sans flex overflow-hidden pointer-events-none">
      <GalleryOsSidebar active="artworks" />

      <div className="flex-1 flex flex-col overflow-hidden">
        <PageHeader
          title="Œuvres"
          sub="127 au total · 89 disponibles · 14 vendues"
          action={
            <PrimaryButton>
              <Icon.Plus size={9} />
              Ajouter
        ...
        <div className="px-4 md:px-5 pb-2 border-b border-zinc-100">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-1">
            {STATUS_PILLS.map((p, i) => (
              <span
                key={p}
                className={`px-1.5 py-[1px] text-[8px] rounded-full border ${i === 0 ? "bg-zinc-900 text-white border-zinc-900" : "bg-white text-zinc-700 border-zinc-200"}`}
              >
                {p}
              </span>
            ))}
            ...
            <div className="ml-auto inline-flex rounded-md border border-zinc-200 overflow-hidden">
              <span className="px-1.5 py-[2px] bg-zinc-900 text-white">
                <Icon.Grid size={9} />
              </span>
              <span className="px-1.5 py-[2px] bg-white text-zinc-500 border-l border-zinc-200">
                <Icon.List size={9} />
              </span>
            </div>
          </div>
          <p className="mt-1.5 text-[8px] text-zinc-400">5 résultats · 127 au total</p>
        </div>

        {/* Grid */}
        <div className="flex-1 overflow-hidden px-4 md:px-5 py-3">
          <div className="grid grid-cols-5 gap-2">
            {ARTWORK_ROWS.map((a) => (
              ...
              >
                <div className="aspect-square w-full relative" style={{ background: a.bg }} />
