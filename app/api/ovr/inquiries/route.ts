...
function slotLabel(slot: InquirySlot) {
  return [slot.artist, slot.title, slot.year].map(v => v?.trim()).filter(Boolean).join(', ') || 'Artwork inquiry'
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const notifyTo = validEmail(vr.inquiryEmail) ? vr.inquiryEmail!.trim() : validEmail(vr.ownerEmail) ? vr.ownerEmail!.trim() : ''
...
<html><body style="margin:0;padding:32px;background:#ffffff;font-family:Arial,Helvetica,sans-serif;color:#111111;">
  ...
  <p style="margin:10px 0 0;font-size:14px;"><a href="${escapeHtml(dashboardUrl)}" style="color:#111111;text-decoration:underline;">Open dashboard</a></p>
    ...
    return NextResponse.json({ ok: true, inquiryId: inquiry._id, notificationSent }, { status: 201 })
  } catch (err) {
