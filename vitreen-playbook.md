...
**Status: current positioning, decided 2026-08-11. Replaces the previous
"founder-led productized service" playbook** (single product, Setup/Partner
pricing, "the agent is the product" framing). That material is not deleted —
it now describes **Vitreen Layer** specifically, one of three branches below —
but it no longer describes the whole company. If you find material treating
Vitreen as one product with one pricing table, that framing is obsolete for
company-level decisions; re-scope it to Layer before using it.
...
**Note (2026-08-11):** the old Setup/Partner pricing detailed in the retired
playbook version was Layer's commercial model specifically (installation +
monthly, plus an optional Partner monthly add-on). It still applies to Layer
as-is until revisited. Studio and Gallery Assistant have no validated pricing
yet — do not invent numbers for them.
...
**Note (2026-08-11):** the current EN/FR homepage (`app/(en)/page.tsx`,
`app/(fr)/fr/page.tsx`) is still structured as a single-product funnel — it
describes Layer only. It has not yet been rebuilt around this 3-branch
architecture. Treat that as open work, not as a contradiction to resolve
silently.
...
For Layer's detailed positioning, mechanism, pricing (Setup/Partner) and
capacity model — all still valid, just re-scoped from "the company" to "this
branch" — see `CLAUDE.md` §1.
...
No detailed positioning work done yet beyond this playbook — do not invent
services, pricing or a website structure for Studio without checking with the
user first.
...
Overlaps heavily with what CLAUDE.md §1/§10 (below, Layer-scoped) already
describes as "the Sales Agent" — the existing agent in `gallery-OS/dashboard/`
is likely the seed of this branch, not a separate build. Confirm with the user
before treating them as unrelated.
